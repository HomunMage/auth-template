<!-- src/routes/callback/android/google/+page.svelte -->
...
<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			const redirectUri = `${apiBase}/callback/android/google`;

			// Exchange code for tokens via backend
			const tokenResp = await fetch(`${apiBase}/api/login/google/token`, {
				method: 'POST',
